...
- [#705](https://github.com/kpdecker/jsdiff/pull/705) **filenames in `Index:` headers generated by `createPatch` et al. now get quoted and escaped if they contain special characters, and quoted index headers are likewise now unquoted and unescaped by `parsePatch`** just like filenames in `+++`/`---` headers. This prevents `createPatch` from emitting patches that `parsePatch` cannot parse back (in the perverse case where a filename contains a line feed character).
